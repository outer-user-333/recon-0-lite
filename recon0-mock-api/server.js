...
//         console.log(
//             `SUCCESS: PATCH /api/v1/reports/${req.params.id}/status - New status: ${status}`
//         );
//         res.json({ success: true, data: mockReports[reportIndex] });
//     } else {
//         console.error(
//             `ERROR: PATCH /api/v1/reports/${req.params.id}/status - Report not found or status missing`
//         );
//         res
//             .status(404)
//             .json({
//                 success: false,
//                 message: "Report not found or status not provided",
//             });
//     }
// });
...
app.use((req, res, next) => {
    ...
    res.status(404).json({
        success: false,
        message: `Endpoint ${req.originalUrl} not found on the mock server.`,
    });
});
