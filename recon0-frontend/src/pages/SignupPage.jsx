  ...
  return (
    ...
    <div className="min-h-screen bg-slate-200 flex flex-col justify-center items-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 p-8 relative z-10">
        <Logo />
        <h2 className="text-3xl font-bold text-center text-slate-900 mb-1">Welcome!</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <button type="button" onClick={() => setRole('hacker')} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-300 ${role === 'hacker' ? 'border-emerald-500 bg-emerald-500 text-white ring-4 ring-emerald-200 shadow-lg transform scale-105' : 'border-slate-300 bg-white hover:bg-emerald-50 hover:border-emerald-300'}`}>
                        <User className={`h-8 w-8 mb-2 ${role === 'hacker' ? 'text-white' : 'text-emerald-500'}`} />
                        <span className={`font-semibold ${role === 'hacker' ? 'text-white' : 'text-emerald-600'}`}>Hacker</span>
                    </button>
                    <button type="button" onClick={() => setRole('organization')} className={`flex flex-col items-center justify-center p-4 rounded-xl border-2 transition-all duration-300 ${role === 'organization' ? 'border-orange-500 bg-orange-500 text-white ring-4 ring-orange-200 shadow-lg transform scale-105' : 'border-slate-300 bg-white hover:bg-orange-50 hover:border-orange-300'}`}>
                        <Building2 className={`h-8 w-8 mb-2 ${role === 'organization' ? 'text-white' : 'text-orange-500'}`} />
                        <span className={`font-semibold ${role === 'organization' ? 'text-white' : 'text-orange-600'}`}>Organization</span>
                    </button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="fullName" className="block text-sm font-medium text-slate-900">Full Name</label>
                    <input id="fullName" type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="mt-1 block w-full px-4 py-3 bg-blue-50 border-2 border-blue-300 rounded-lg shadow-sm placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"/>
                </div>
                <div>
                    <label htmlFor="username" className="block text-sm font-medium text-slate-900">Username</label>
                    <input id="username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} required className="mt-1 block w-full px-4 py-3 bg-violet-50 border-2 border-violet-300 rounded-lg shadow-sm placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 transition-all"/>
                </div>
            </div>

            <div>
                <label htmlFor="email" className="block text-sm font-medium text-slate-900">Email Address</label>
                <input id="email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 block w-full px-4 py-3 bg-emerald-50 border-2 border-emerald-300 rounded-lg shadow-sm placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all"/>
            </div>

            <div>
                <label htmlFor="password" className="block text-sm font-medium text-slate-900">Password</label>
                <input id="password" type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="mt-1 block w-full px-4 py-3 bg-rose-50 border-2 border-rose-300 rounded-lg shadow-sm placeholder-slate-400 text-slate-900 focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all"/>
            </div>

            {/* File Upload with orange theme */}
            <div>
                 <label htmlFor="avatarFile" className="block text-sm font-medium text-slate-900">Profile Picture / Logo (Optional)</label>
                 <label htmlFor="avatarFile" className="mt-1 flex justify-center w-full h-32 px-6 pt-5 pb-6 border-2 border-dashed border-orange-400 bg-orange-50 rounded-xl cursor-pointer hover:border-orange-500 hover:bg-orange-100 transition-all">
                    <div className="space-y-1 text-center">
                        ...
                        <p className="text-xs text-orange-600">PNG, JPG, GIF up to 10MB</p>
                    </div>
